...
**Two time variables are always available:**

- `t` — local normalised time, 0→1 over the object's own timeline window. Use this for portable animations: `opacity = t` always fades in over whatever duration the object is given.
- `T` — global absolute time in seconds. Use this for physics where the rate must match real elapsed time: `exp(-1.8 * T)`.
...
214 assertions, all passing. Tests are plain TypeScript — no test framework dependency. If you add a feature, add tests.
...
3. Expressions have access to `t` (local 0→1), `T` (global seconds), `s` (parametric domain), any `params` or `variables` you define, and named `functions`.
...
**t-first.** The default time variable is `t` (local, 0→1). This makes specs portable — changing an object's `timeline.start` and `timeline.end` changes when and how fast the animation plays, not just whether it's visible. `T` (global seconds) is available when you need it for physics.
