...
Equanim separates _what an animation does_ from _how it's rendered_. A spec is pure data — no code, no runtime dependencies, no renderer assumptions. You hand it to a renderer and it plays.
...
**Renderer-agnostic.** The spec says what the animation _is_. It doesn't say how to render it. The coordinate system is documented so any renderer can implement it consistently.
...
- [x] Example specs — dampened wave, bouncing ball, double pendulum
- [x] `ode_system` node — RK4 numerical integration; state variables exposed as interpolators to sibling objects
